...
import { startMeetingHandler } from "./start";
...
describe("PATCH /api/meetings/:id/start", () => {
	it("retorna 401 sem autenticação", async () => {
		...
		const response = await startMeetingHandler({
			request: new Request("http://localhost/api/meetings/meeting-1/start", {
				method: "PATCH",
			}),
	...
	it("resolve env via fallback quando o contexto não traz env", async () => {
		...
		const request = new Request(
			"http://localhost/api/meetings/meeting-1/start",
			{ method: "PATCH" },
		);
		const response = await startMeetingHandler({
			request,
	...
	it("retorna 404 quando a reunião não existe", async () => {
		...
		const response = await startMeetingHandler({
			request: new Request("http://localhost/api/meetings/missing/start", {
				method: "PATCH",
			}),
			context: { env: createEnv() },
	...
	it("retorna 422 quando a reunião não tem turmas", async () => {
		const sessionMock = getSession as ReturnType<typeof vi.fn>;
		sessionMock.mockResolvedValueOnce(createMockSession());
		(findMeetingById as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
			id: "meeting-1",
		...
		(transitionMeeting as ReturnType<typeof vi.fn>).mockRejectedValueOnce(
			new MeetingWithoutClassesError(
	...
	it("retorna 409 quando a reunião já está em andamento", async () => {
		const sessionMock = getSession as ReturnType<typeof vi.fn>;
		sessionMock.mockResolvedValueOnce(createMockSession());
		(findMeetingById as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
			id: "meeting-1",
	...
	it("retorna 200 e inicia a reunião", async () => {
		const sessionMock = getSession as ReturnType<typeof vi.fn>;
		sessionMock.mockResolvedValueOnce(createMockSession());
		(findMeetingById as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
			id: "meeting-1",
