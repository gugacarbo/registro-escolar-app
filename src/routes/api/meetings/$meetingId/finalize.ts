...
export const Route = createFileRoute("/api/meetings/$meetingId/finalize")({
	server: {
		middleware: [d1Middleware],
		handlers: {
			PATCH: finalizeMeetingHandler,
		},
	},
});
...
export async function finalizeMeetingHandler({
	request,
	context,
	params,
}: {
	...
	try {
		const finalized = await transitionMeeting(db, params.meetingId, "finalize");
