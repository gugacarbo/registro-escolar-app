...
import { finalizeMeetingHandler } from "./finalize";
...
describe("PATCH /api/meetings/:id/finalize", () => {
	it("retorna 401 sem autenticação", async () => {
		const sessionMock = getSession as ReturnType<typeof vi.fn>;
		sessionMock.mockResolvedValueOnce(null);
		const response = await finalizeMeetingHandler({
			request: new Request("http://localhost/api/meetings/meeting-1/finalize", {
				method: "PATCH",
			}),
	...
	it("resolve env via fallback quando o contexto não traz env", async () => {
		...
		const request = new Request(
			"http://localhost/api/meetings/meeting-1/finalize",
			{ method: "PATCH" },
		);
		const response = await finalizeMeetingHandler({
			request,
	...
	it("retorna 404 quando a reunião não existe", async () => {
		...
		const response = await finalizeMeetingHandler({
			request: new Request("http://localhost/api/meetings/missing/finalize", {
				method: "PATCH",
			}),
			context: { env: createEnv() },
	...
	it("retorna 409 quando a reunião ainda é rascunho", async () => {
		const sessionMock = getSession as ReturnType<typeof vi.fn>;
		sessionMock.mockResolvedValueOnce(createMockSession());
		(findMeetingById as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
			id: "meeting-1",
	...
	it("retorna 200 e finaliza a reunião", async () => {
		const sessionMock = getSession as ReturnType<typeof vi.fn>;
		sessionMock.mockResolvedValueOnce(createMockSession());
		(findMeetingById as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
			id: "meeting-1",
